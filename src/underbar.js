(function() {
  ...
  // Produce a duplicate-free version of the array. _.uniq([1,2,3], , function)
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (typeof arguments[1] === 'function') {
      for (let i = 0; i < array.length; i ++) {
        var test = isSorted(array[i]);
        if (!hold.includes(test)) {
          hold.push(test);
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(ele, ind, collection) {
      result.push(iterator(ele, ind, collection));
    });
  ...
  _.every = function(collection, iterator) {
    ...
    return !!_.reduce(collection, function(trueSoFar, ele) {
      return trueSoFar && iterator(ele);
    }, true);
  };
  ...
  _.shuffle = function(array) {
    ...
    while (copyArray.length > 0) {
      ...
      result.push(copyArray.splice(randIndex, 1)[0]);
    }

    return result;

  };
  //eslint --fix underbar.js/
  ...
  _.zip = function() {
    var result = [];
    var args = Array.from(arguments);
    var sizes = _.map(args, function(elem) {
      return elem.length;
    });
  ...
  _.intersection = function() {
    var results = [];
    var args = Array.from(arguments);
    // each element of each array and check if they exist in the other arrays
    _.each(args[0], function(val) {
      var includes = _.every(args, function(arg) {
